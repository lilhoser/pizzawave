...
#### Controlled A-B-A shared-Paxan capacity comparison

On 2026-07-24, OT and RPI completed a bounded three-phase comparison using
20-minute half-open windows. Commit `e50b246` added duration telemetry for
every incident-related completion request on OT. RPI was not deployed or
reconfigured; its serialized request starts from the service journal were
reconciled fail-closed with its durable `lm_usage` completions. One RPI
extraction began 20.703 seconds before the B boundary and completed inside the
window, so only its 34.363 seconds of in-window occupancy was counted. The
reconciliation tool now handles that boundary case explicitly and reports it
as a clipped leading request.

| Phase | Window (UTC) | OT pipeline | Usable observations (OT + RPI) | Requests (OT + RPI) | Tokens (OT + RPI) | Request occupancy |
| --- | --- | --- | ---: | ---: | ---: | ---: |
| A1 | 12:40:13–13:00:13 | legacy | 85 + 38 | 20 + 6 | 39,365 + 11,181 | 53.5% |
| B | 13:03:34–13:23:34 | non-persistent replacement | 100 + 40 | 30 + 16 | 94,438 + 29,817 | 96.1% |
| A2 | 13:28:25–13:48:25 | legacy | 114 + 26 | 18 + 12 | 32,096 + 24,457 | 63.3% |

All 102 measured requests succeeded. During B, OT used 7 constructor requests,
6 relationship requests, and 17 standalone-verification requests. Those
requests occupied 686.350 seconds of the 1,200-second window; RPI legacy
occupied another 466.732 seconds. The constructor processed 91 of the 100
usable OT observations inside the window, produced 17 valid verifier results,
and recorded no validation errors. Persistence and production ownership were
disabled. The B production baseline remained exactly 5,800 incidents, maximum
incident ID 7,215, and 54,379 incident-operation audits.

B and A2 both observed 140 usable calls, or 7.0 calls per minute across the two
systems, so A2 is the strongest demand-matched control. Combined measured
request occupancy increased from 63.3% under legacy/legacy to 96.1% under
replacement/legacy. A1 had slightly lower aggregate demand at 6.15 calls per
minute and 53.5% occupancy. The comparison therefore establishes that the
current replacement request fan-out consumes materially more shared Paxan time
than OT legacy at comparable demand. It does **not** establish that RPI was
starved: RPI remained current, both health endpoints stayed `ok`, no model
request failed, and neither recorder changed during the experiment.

The full-occupancy replay used the measured replacement cost of 7.542 seconds
and 1,037.8 tokens per processed observation. At the proven demand of 7.0
observations per minute, replacing both systems projects 88.0% request
occupancy. The required 1.5-times headroom case projects 132.0%, so it fails
the production capacity gate. That projection is not a measured RPI
replacement run and must not be treated as one.

The production decision is to leave OT on legacy and keep every replacement
execution, persistence, and ownership switch disabled. The replacement is
grounded and functionally viable, but its present inference fan-out has
essentially no burst headroom beside RPI legacy. The next bounded engineering
step is to reduce request fan-out while preserving independent evidence
verification, starting by verifying all standalone events and proposed
relationships from one constructor batch in a single typed verifier request
instead of issuing one standalone request per event. The same B window must be
repeated after that change. If the measured replacement/legacy combination
still cannot stay below the headroom boundary, shared inference scheduling or
additional capacity becomes an explicit production design decision rather
than an assumption made during cutover.

After A2, OT remained on legacy with constructor, relationship, verifier,
exclusive-window, canary, and production-ownership flags disabled. At the
post-experiment checkpoint, OT and RPI health were `ok`, both transcription and
embedding queues were clear, and AI completion failures were zero. OT
`trunk-recorder` remained PID `2110751` with restart count 5; RPI remained PID
`884754` with restart count 0.

#### Demand-reduction implementation and frozen semantic gate

The operator subsequently accepted a longer ordinary publication delay (about
10 minutes, and longer when it materially reduces load), independent
standalone publication before later relationship reconciliation, and two
independent semantic passes only for automatic merges. Incorrectly combining
unrelated calls remains the worst error; missing a valid incident is next. A
bounded backlog is acceptable, but the replacement must serve OT, RPI, and
future systems through the existing shared Paxan/Qwen deployment.

The implementation behind
`aiInsights.incidentBatchExhaustiveBatchedIntakeEnabled` adds the intended
low-fan-out boundary without enabling it in production:

- deterministic, source-isolated exhaustive intake emits one provisional
  singleton for every new observation without a constructor model request;
- application-owned evidence and fail-closed complete row coverage prevent
  observations from disappearing before semantic review;
- one compact standalone-verification request decides all provisional
  singletons from a constructor batch, with explicit one-based row identities
  and row-local evidence-span indices;
- one confirmation request decides all pending relationship proposals from a
  constructor batch, while preserving an independent relationship proposer and
  verifier for every automatic merge;
- support for 24 source observations and a maximum batch dwell of 900 seconds;
- replacement-only, audited merge handling that cannot absorb a legacy-owned
  call;
- durable previous-title and merge-target fields, plus `Updated` and
  `Combined` dashboard badges. The old title is available in the Updated badge
  tooltip and a merged card points to its canonical incident;
- verifier-generated titles are limited to 80 characters at a word boundary.

A single model source-classification pass was implemented and then rejected by
the frozen semantic gate. It matched only 8 of 12 expected decisions, including
two routine-status false positives and two missed or deferred explicit
structure-fire observations. Direct publication from that classifier was
removed. The enabled demand-reduced profile now uses deterministic exhaustive intake
followed by the batched independent standalone verifier, so the failed
classifier cannot become a production authority.

The same frozen 12-observation gate passed at the incident level under the
batched verifier. All six routine/status observations were rejected. Five of
six clear incident observations were independently Verified. The remaining
call, reporting occupants out while flames remained around a microwave, was
rejected as a standalone incident, but it was a continuation of the same
structure fire represented by other Verified calls in the batch and remains
eligible for the independently proposed and verified relationship path. No
underlying incident in the fixture was lost. The request used 1,620 prompt
tokens, 448 completion tokens, and 2,068 total tokens, completing in 54.776
seconds while sharing the live server. The structural tests also reject
missing, shifted, extra, or cross-owned rows.

Relative to the measured 20-minute B window, deterministic intake removes all
seven constructor requests (352.203 request-seconds and 24,763 tokens). Five
24-observation standalone batches would be approximately 20,680 tokens using
the frozen gate's observed token density, compared with 17 per-event verifier
requests and 14,049 tokens in B. At the measured relationship-stage rate,
batched relationship confirmation reduces six relationship-verifier requests
to about five while preserving both semantic passes for merges. This projects
OT replacement demand at roughly 67,000 tokens per 20 minutes instead of
94,438, about 29% lower, and combined OT-plus-RPI demand at roughly 97,000
instead of 124,255, about 22% lower. This is a sizing projection, not a
production capacity result: live request durations overlap under parallelism
four and cannot be added as exclusive GPU occupancy.

Paxan's live LM Studio configuration was inspected read-only. The resident
`qwen/qwen3.6-35b-a3b@q8_0` instance has a 65,536-token context, evaluation and
physical batch sizes of 512, parallelism 4, flash attention enabled, eight
experts configured, GPU KV-cache offload enabled, and speculative decoding
disabled. The model advertises a 262,144-token maximum, so the configured 65K
context is conservative and comfortably accommodates the bounded prompts. The
settings were not changed.

The implementation and frozen correctness gate pass. The deployment gate
remains closed until one fixed, non-persistent OT shadow measures this exact
profile under ordinary shared Paxan demand: 24-observation batches, a 600-900
second dwell, deterministic intake, one batched standalone verification, and
batched relationship confirmation. That run should be judged once against the
already frozen A1/B/A2 control on processed-call currency, model failures,
combined request demand, and both systems' queue age. No model bake-off or
architecture pivot is part of that gate. If it stays current without degrading
RPI, the next action is a phased OT production cutover followed by RPI; if it
does not, the evidence points to scheduling or additional inference hardware
rather than further semantic prompt experiments.
...
## 2026-07-26 verified standalone production ownership

The demand-reduced shadow automation created on 2026-07-25 did not execute its
follow-up turns. The OT shadow therefore remained active beyond its fixed stop
time while legacy incident execution was disabled. It stayed non-persistent,
but accumulated 381 batches and left legacy completion health stale. Recovery
re-enabled legacy execution, disabled every replacement flag, and allowed the
existing stale-work policy to retain old jobs as `skipped_stale`; no production
rows were manually deleted or backfilled. The inactive heartbeat was deleted.

That overdue evidence exposed a deterministic handoff defect rather than a
model-quality problem. Exhaustive intake correctly emitted one provisional
singleton for every observation, but per-event acceptance reapplied the
whole-batch exhaustive-coverage rule to each singleton. All singletons were
therefore rejected before batched standalone verification. Commit `71149f0`
separates whole-batch coverage validation from per-event validation and adds a
regression proving that exhaustive singletons remain accepted and enqueue one
standalone verification request each.

The corrected non-persistent OT preflight processed 24 observations, accepted
all 24 source events, completed one batched standalone verification, and
returned one grounded Verify plus 23 Reject decisions with zero invalid
results. Production incident and audit counts did not change. The same preflight
also measured the relationship request at 33,255 tokens and 96.7 seconds,
versus 4,910 tokens and 50.5 seconds for the standalone batch. Running both on
every full batch would exceed the intended shared-Paxan budget, so relationship
inference was not promoted to production.

Commit `75bb926` permits the audited writer to accept a Verified standalone
result from exhaustive, observation-isolated, batched intake without requiring
the relationship stage. The relationship path still retains its original
staged-verification gate when it is enabled. The complete test suite passes
738/738 tests.

OT entered permanent `production-incident-standalone-v1` ownership above call
1498740 on 2026-07-26 at approximately 18:23 UTC. Legacy execution and
relationship inference are disabled. Exhaustive deterministic intake, batched
standalone verification, exclusive inference ownership, audited persistence,
and permanent replacement ownership are enabled. The first production batch:

- processed and verified 24 observations in one model request;
- used 3,467 prompt tokens, 756 completion tokens, 4,223 total tokens, and
  59.3 seconds;
- returned six Verify and eighteen Reject decisions with zero Review, invalid,
  or conflict outcomes;
- persisted incidents 7330 through 7335 through the audited writer, with an
  exact one-for-one incident/audit delta;
- left OT and RPI completion, transcription, embedding, and recorder health
  stable. RPI remained on its existing legacy pipeline and was not deployed or
  reconfigured.

This production boundary was rolled back at approximately 18:43 UTC after five
batches made the product failure explicit. It persisted 34 mostly single-call
incidents, fragmenting real incidents into transmission-sized records; many
fragments also lacked enough context to support useful geolocation. The
replacement flags were disabled and legacy execution was restored without
manually deleting or rewriting those audited records. Commit `d67daa9` removes
the standalone-production authorization so configuration alone cannot re-enable
this failure mode. A future replacement must group evidence into incident-level
objects before persistence. Incrementally grouping already-persisted singleton
incidents is not an acceptable substitute because it preserves the same
cardinality and context-loss problem downstream.

At the operator's explicit request, the 34 fragmented production incidents
created by `production-incident-standalone-v1` were removed in one verified
transaction. The target set was resolved exclusively through the run's
append-only canary commits; every live incident key and one-call ownership row
had to match its commit payload before deletion. Incidents 7330 through 7363
and their 34 `incident_calls` links were deleted. No other incident, call link,
operation audit, or shadow/canary ledger row changed. The 34 canary commits were
preserved as the historical record. The database moved from 5,948 to 5,914
incidents and from 12,880 to 12,846 incident-call links while operation audits
remained at 55,503. The recoverable pre-cleanup backup is
`/var/lib/pizzawave/pizzad.db.pre-fragmented-incident-cleanup-20260726T184931Z.bak`.
