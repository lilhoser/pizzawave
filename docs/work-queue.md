...
- Initial-collapse flight recording and paired-wide analysis are complete. RPI
  ...
  The incident membership-model project remains independently owned and must
  not be merged or deployed as part of RF work.
...
- Thread source-control reconciliation confirmed that all Package 5-11,
  ...
  branches and fully merged remote branches were retired. Historical incident
  experiments are archived; new incident work starts from the authoritative
  architecture and training handoff on `main`.
...
- Retired incident research is preserved by
  `archive/incident-research-tooling-20260717`.
- The superseded platform refactor is preserved by
  `archive/platform-refactor-legacy`.
- Neither has an active worktree or branch and neither should be merged into
  the current product.
