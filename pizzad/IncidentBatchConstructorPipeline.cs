...
public sealed class ApplicationIncidentBatchExhaustiveSourceProposer(TimeProvider? timeProvider = null)
    : IIncidentBatchProposer
{
    public const string PromptIdentity = "application:incident-batch-exhaustive-source-intake-v1";
        ...
        CancellationToken ct)
    {
        if (candidates.Count > 0)
            throw new ArgumentException("deterministic source intake cannot receive candidates", nameof(candidates));
        var catalog = IncidentBatchConfirmationEvidenceCatalog.Build(bundle);
        var events = newObservationIds.Select(observationId =>
        {
            var evidence = catalog
                .Where(item => item.ObservationId == observationId)
                .Select(item => new IncidentEventStateTranscriptCitation(item.TranscriptId, item.ExactQuote))
                .Take(IncidentBatchRelationshipContract.MaximumEvidenceSpansPerSide)
                .ToList();
            return new IncidentBatchEventProposal(
                $"source:{observationId}",
                IncidentBatchEventDisposition.ProvisionalEvent,
                string.Empty,
                [observationId],
                IncidentBatchProjector.BuildEvidenceTitle(IncidentBatchProjector.BuildEvidenceSummary(evidence)),
                IncidentBatchProjector.BuildEvidenceSummary(evidence),
                "Awaiting batched independent standalone verification.",
                0.5,
                evidence,
                [],
                [],
                ["Batched standalone verification is pending."]);
        }).ToList();
        return Task.FromResult(new IncidentBatchProposal(
            $"application:exhaustive-source:{Guid.NewGuid():N}",
...
public static class IncidentBatchContract
{
    ...
    public const string ExhaustiveSourceIntakeConfigurationToken = "source-intake=exhaustive-v1";
    ...
    public static IncidentEventStateContractValidationResult ValidateInput(
        ...
        IReadOnlyList<string> newObservationIds,
        IReadOnlyList<IncidentBatchCandidate> candidates,
        bool allowPendingReconsideration = false)
    {
        ...
        foreach (var candidate in candidates)
        {
            ...
            foreach (var observationId in candidate.ObservationIds)
            {
                ...
                if (newObservationIds.Contains(observationId, StringComparer.Ordinal) &&
                    (!allowPendingReconsideration || candidate.OperatorVisible))
        ...
        string configurationIdentity = "")
    {
        ...
        if (UsesExhaustiveSourceIntake(configurationIdentity))
            ValidateExhaustiveSourceIntake(newObservationIds, proposal.Events, errors);
        ...
        string configurationIdentity)
    {
        ...
        if (UsesExhaustiveSourceIntake(configurationIdentity))
        {
            var exhaustiveErrors = new List<string>();
            ValidateExhaustiveSourceIntake(newObservationIds, proposal.Events, exhaustiveErrors);
            if (exhaustiveErrors.Count > 0)
                return [];
        }
        ...
        var headerValidation = ValidateProposal(
            ...
            RemoveExhaustiveSourceIntake(configurationIdentity));
        ...
        return events
            ...
            .Where(item => ValidateProposal(
                ...
                RemoveExhaustiveSourceIntake(configurationIdentity)).IsValid)
            .ToList();
    }

    private static void ValidateExhaustiveSourceIntake(
        IReadOnlyList<string> newObservationIds,
        IReadOnlyList<IncidentBatchEventProposal> events,
        List<string> errors)
    {
        var proposedObservationIds = events.SelectMany(item => item.NewObservationIds).ToList();
        foreach (var missing in newObservationIds.Except(proposedObservationIds, StringComparer.Ordinal))
            errors.Add($"source intake omitted new observation '{missing}'");
        foreach (var unknown in proposedObservationIds.Except(newObservationIds, StringComparer.Ordinal))
            errors.Add($"source intake returned unknown observation '{unknown}'");
        foreach (var item in events.Where(item => item.Disposition != IncidentBatchEventDisposition.ProvisionalEvent))
            errors.Add($"exhaustive source intake event '{item.ProposalToken}' must remain provisional until batched verification");
    }

    private static string RemoveExhaustiveSourceIntake(string configurationIdentity) =>
        string.Join(';', configurationIdentity
            .Split(';', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
            .Where(token => !string.Equals(token, ExhaustiveSourceIntakeConfigurationToken, StringComparison.Ordinal)));
    ...
    public static bool UsesExhaustiveSourceIntake(string configurationIdentity) =>
        configurationIdentity.Split(';', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
            .Contains(ExhaustiveSourceIntakeConfigurationToken, StringComparer.Ordinal);
...
public static class IncidentBatchProjector
{
        ...
        DateTimeOffset generatedAtUtc)
    {
        ...
        var asynchronousProvisional = IncidentBatchExecutionArchitecture.UsesAsynchronousProvisionalVerification(
            entry.Execution.ConfigurationIdentity);
        var rollingSinglePass = entry.Execution.ConfigurationIdentity
            .Split(';', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
            .Contains(IncidentRollingHypothesis.ExecutionToken, StringComparer.Ordinal);
        var priorOwners = events
            .SelectMany(item => item.ObservationIds.Select(observationId => (Event: item, ObservationId: observationId)))
            .Where(item => entry.NewObservationIds.Contains(item.ObservationId, StringComparer.Ordinal))
            .ToList();
        if (priorOwners.Any(item => !rollingSinglePass || item.Event.OperatorVisible || item.Event.OperatorReview))
            throw new InvalidOperationException("batch contains an observation already owned by a published prior projection");
        if (priorOwners.Count > 0)
        {
            var reopened = priorOwners.Select(item => item.ObservationId).ToHashSet(StringComparer.Ordinal);
            events = events
                .Select(item => item with
                {
                    ObservationIds = item.ObservationIds.Where(observationId => !reopened.Contains(observationId)).ToList()
                })
                .Where(item => item.ObservationIds.Count > 0)
                .ToList();
        }
        ...
        foreach (var proposal in IncidentBatchContract.AcceptedEvents(entry))
        {
            ...
            var evidenceSummary = rollingSinglePass ? proposal.Summary.Trim() : BuildEvidenceSummary(proposal.NewObservationEvidence);
            var evidenceTitle = rollingSinglePass ? proposal.Title.Trim() : BuildEvidenceTitle(evidenceSummary);
            ...
            else
            {
                ...
                events[sourceIndex] = source with
                {
                    ...
                    OperatorVisible = (rollingSinglePass && proposal.Disposition == IncidentBatchEventDisposition.NewEvent) || IncidentBatchContract.IsOperatorVisibleNewEvent(proposal),
                    OperatorReview = !rollingSinglePass && (IncidentBatchContract.IsOperatorReviewEvent(proposal) || requiresAsynchronousVerification),
...
public sealed class IncidentBatchCoordinator
{
    ...
    public async Task<IncidentBatchRunResult> RunAsync(
        ...
        CancellationToken ct)
    {
        var rollingSinglePass = request.ConfigurationIdentity
            .Split(';', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
            .Contains(IncidentRollingHypothesis.ExecutionToken, StringComparer.Ordinal);
        var inputValidation = IncidentBatchContract.ValidateInput(
            bundle,
            priorProjection,
            newObservationIds,
            candidates,
            allowPendingReconsideration: rollingSinglePass);
