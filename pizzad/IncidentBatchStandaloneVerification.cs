...
public static class IncidentTitlePresentation
{
    public static string Normalize(string? value)
    {
        var title = (value ?? string.Empty).Trim();
        if (title.Length <= IncidentBatchConfirmationContract.MaximumDisplayTitleLength)
            return title;
        title = title[..IncidentBatchConfirmationContract.MaximumDisplayTitleLength].TrimEnd();
        var lastSpace = title.LastIndexOf(' ');
        return lastSpace >= title.Length / 2 ? title[..lastSpace].TrimEnd(' ', ',', '.', ':', ';', '-') : title;
    }
}
...
public static class IncidentBatchStandaloneVerificationContract
{
        ...
        IncidentBatchStandaloneVerificationProposal proposal)
    {
        ...
        if (!string.Equals(proposal.PromptIdentity, IncidentBatchGraphVerification.PromptIdentity, StringComparison.Ordinal) &&
            !string.Equals(proposal.PromptIdentity, IncidentBatchStandaloneBatchVerification.PromptIdentity, StringComparison.Ordinal) &&
            !string.Equals(proposal.PromptIdentity, IncidentBatchStandaloneVerificationPrompt.PromptIdentity, StringComparison.Ordinal) &&
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            var decision = new IncidentBatchStandaloneVerificationDecision(
                ...
                parsed.ConcreteIncidentSupported,
                parsed.ConcreteIncidentSupported ? IncidentTitlePresentation.Normalize(parsed.DisplayTitle) : string.Empty,
