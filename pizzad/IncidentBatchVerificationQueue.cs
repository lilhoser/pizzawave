...
public static class IncidentBatchVerificationQueueContract
{
    public static IReadOnlyList<IncidentBatchVerificationRequest> BuildRequests(IncidentBatchLedgerEntry entry)
    {
        ...
        if (entry.RelationshipProposal is not null)
        {
            ...
            // The two-pass graph verifier must judge every source both as part of the
            // proposed graph and on its own merits. A proposed edge is not allowed to
            // suppress a valid incident merely because the first pass later proves
            // that edge wrong. Persistence still gives a verified component precedence
            // over its member standalones.
            var standaloneExclusions = IncidentBatchGraphVerification.IsEnabled(entry.Execution.ConfigurationIdentity)
                ? new HashSet<string>(StringComparer.Ordinal)
                : relatedObservationIds;
            return BuildStandaloneRequests(entry, acceptedEvents, standaloneExclusions)
