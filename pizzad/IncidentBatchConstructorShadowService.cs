    ...
    IReadOnlyList<IncidentBatchCandidate> Candidates)
{
        ...
        DateTimeOffset createdAtUtc)
    {
        ...
        var groups = (priorProjection?.Events ?? [])
            ...
            .Where(item => item.SourceCalls.Count > 0 && item.SourceCalls.Any(source => double.IsFinite(source.Score)))
            .OrderByDescending(item => item.SourceCalls.Max(source => source.Score))
