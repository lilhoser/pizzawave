...
public sealed class SettingsValidationService
{
    ...
    public SettingsValidationService(EngineConfig config, EngineDatabase database, CredentialStore credentials, ILogger<SettingsValidationService> logger)
    {
        _config = config;
        _database = database;
    ...
    public async Task<object> TestAsync(string section, CancellationToken ct)
    {
        try
        {
            return section.Trim().ToLowerInvariant() switch
            {
                ...
                "embeddings" => await TestEmbeddingsAsync(ct),
                "alerts" => TestAlerts(),
    ...
    public async Task<object> ListAiInsightModelsAsync(CancellationToken ct)
    {
        ...
        var managementBaseUrl = DeriveLmStudioManagementBaseUrl(baseUrl);
        if (!string.IsNullOrWhiteSpace(managementBaseUrl))
        {
            try
            {
                using var nativeResponse = await client.GetAsync($"{managementBaseUrl}/models", ct);
                var nativeText = await nativeResponse.Content.ReadAsStringAsync(ct);
                if (nativeResponse.IsSuccessStatusCode)
                {
                    var nativeModels = ParseLmStudioModelKeys(nativeText);
                    if (nativeModels.Count > 0)
                        return new { ok = true, message = $"Found {nativeModels.Count} LM Studio model id(s).", models = nativeModels };
                }
            }
            catch (Exception ex)
            {
                _logger.LogDebug(ex, "LM Studio native model discovery failed for {BaseUrl}; falling back to OpenAI-compatible /models", baseUrl);
            }
        }
    ...
    private static List<string> ParseLmStudioModelKeys(string text)
    {
        using var doc = JsonDocument.Parse(text);
        var models = new List<string>();
        if (!doc.RootElement.TryGetProperty("models", out var rows) || rows.ValueKind != JsonValueKind.Array)
            return models;

        foreach (var row in rows.EnumerateArray())
        {
            if (row.TryGetProperty("type", out var type) &&
                !string.Equals(type.GetString(), "llm", StringComparison.OrdinalIgnoreCase))
                continue;
            if (row.TryGetProperty("key", out var key) && !string.IsNullOrWhiteSpace(key.GetString()))
                models.Add(key.GetString()!);
            if (row.TryGetProperty("variants", out var variants) && variants.ValueKind == JsonValueKind.Array)
            {
                foreach (var variant in variants.EnumerateArray())
                    if (!string.IsNullOrWhiteSpace(variant.GetString()))
                        models.Add(variant.GetString()!);
            }
        }
    ...
    private static string DeriveLmStudioManagementBaseUrl(string openAiBaseUrl)
    {
        if (!Uri.TryCreate(openAiBaseUrl.TrimEnd('/'), UriKind.Absolute, out var uri))
            return string.Empty;
        var builder = new UriBuilder(uri) { Query = string.Empty, Fragment = string.Empty };
        var path = builder.Path.TrimEnd('/');
        if (path.EndsWith("/v1", StringComparison.OrdinalIgnoreCase))
            path = path[..^3].TrimEnd('/');
        else if (!path.EndsWith("/api/v1", StringComparison.OrdinalIgnoreCase))
            return string.Empty;
        if (!path.EndsWith("/api/v1", StringComparison.OrdinalIgnoreCase))
            path = $"{path}/api/v1".TrimEnd('/');
    ...
    private async Task<object> TestTranscriptionAsync(CancellationToken ct)
    {
        ...
        if (provider == "none")
            return new { ok = false, message = "Transcription is required. Choose and configure a transcription engine before saving this deployment." };
        ...
        if (provider is "lmstudio" or "openai" or "remote-faster-whisper")
        {
            var call = await FindRecentAudioCallAsync(ct);
            if (call == null)
            {
    ...
    private async Task<object> TestEmbeddingsAsync(CancellationToken ct)
    {
        if (!_config.Embeddings.Enabled)
            return new { ok = true, message = "Embeddings are disabled." };
        var baseUrl = (_config.Embeddings.OpenAiBaseUrl ?? string.Empty).TrimEnd('/');
        if (string.IsNullOrWhiteSpace(baseUrl) || string.IsNullOrWhiteSpace(_config.Embeddings.OpenAiModel))
            return new { ok = false, message = "Embedding base URL and model are required." };
        ...
        var body = JsonSerializer.Serialize(new { model = _config.Embeddings.OpenAiModel, input = "PizzaWave embedding health check" }, EngineConfig.JsonOptions());
        using var response = await client.PostAsync($"{baseUrl}/embeddings", new StringContent(body, Encoding.UTF8, "application/json"), ct);
        var text = await response.Content.ReadAsStringAsync(ct);
        if (!response.IsSuccessStatusCode)
            return new { ok = false, message = $"Embedding endpoint HTTP {(int)response.StatusCode}: {Trim(text, 300)}" };
        using var doc = JsonDocument.Parse(text);
        var dimension = doc.RootElement.GetProperty("data")[0].GetProperty("embedding").GetArrayLength();
        if (dimension != _config.Embeddings.VectorSize)
            return new { ok = false, message = $"Embedding dimension {dimension} does not match configured Qdrant vector size {_config.Embeddings.VectorSize}." };

        using var qdrant = new HttpClient { Timeout = TimeSpan.FromSeconds(10) };
        if (!string.IsNullOrWhiteSpace(_config.Embeddings.QdrantApiKey))
            qdrant.DefaultRequestHeaders.Add("api-key", _config.Embeddings.QdrantApiKey);
        using var qResponse = await qdrant.GetAsync($"{_config.Embeddings.QdrantBaseUrl.TrimEnd('/')}/collections/{Uri.EscapeDataString(_config.Embeddings.Collection)}", ct);
        return new { ok = true, message = qResponse.IsSuccessStatusCode ? $"Embedding endpoint responded and Qdrant collection {_config.Embeddings.Collection} exists." : $"Embedding endpoint responded. Qdrant collection {_config.Embeddings.Collection} will be created by the worker." };
    }
