...
public sealed partial class EngineDatabase : IIncidentBatchStore
{
        ...
        CancellationToken ct)
    {
        ...
        await using (var existingIncident = connection.CreateCommand())
        {
            ...
            existingIncident.CommandText = "SELECT id, title FROM incidents WHERE incident_key=$incident_key;";
            existingIncident.Parameters.AddWithValue("$incident_key", intent.IncidentKey);
            await using var reader = await existingIncident.ExecuteReaderAsync(ct);
            if (await reader.ReadAsync(ct))
            {
                incidentId = reader.GetInt64(0);
                existingTitle = reader.GetString(1);
            }
            else
            {
                incidentId = 0;
        ...
        var conflictingOwners = new List<(long CallId, long IncidentId, string IncidentKey, string Status)>();
        await using (var ownerCommand = connection.CreateCommand())
        {
            ownerCommand.Transaction = transaction;
            ownerCommand.CommandText = $"""
                SELECT ic.call_id, ic.incident_id, COALESCE(i.incident_key, ''), COALESCE(i.status, 'active')
                FROM incident_calls ic
                JOIN incidents i ON i.id=ic.incident_id
                WHERE ic.call_id IN ({string.Join(",", callParameters)})
                  AND ic.incident_id <> $canary_incident_id
                  AND COALESCE(i.status, 'active') <> 'merged'
                ORDER BY ic.call_id, ic.incident_id;
                """;
            for (var index = 0; index < intent.CallIds.Count; index++)
            ...
            while (await reader.ReadAsync(ct))
                conflictingOwners.Add((reader.GetInt64(0), reader.GetInt64(1), reader.GetString(2), reader.GetString(3)));
        }

        var created = incidentId == 0;
        var outcome = IncidentBatchCanaryCommitOutcome.Persisted;
        string reason;
        var mergeOwners = conflictingOwners
            .Where(item => item.IncidentKey.StartsWith("batch-canary:", StringComparison.Ordinal) &&
                           string.Equals(item.Status, "active", StringComparison.OrdinalIgnoreCase))
            .Select(item => item.IncidentId)
            .Distinct()
            .ToList();
        var unsafeConflict = conflictingOwners.Any(item => !mergeOwners.Contains(item.IncidentId));
        if (unsafeConflict || (mergeOwners.Count > 0 && created))
        {
        ...
        else
        {
            ...
            else
            {
                ...
                update.CommandText = """
                    ...
                    SET title=$title,
                        detail=$detail,
                        status='active',
                        previous_title=CASE WHEN title <> $title THEN title ELSE previous_title END,
                        title_updated_at_utc=CASE WHEN title <> $title THEN $now ELSE title_updated_at_utc END,
                        merged_into_incident_id=0,
            ...
            foreach (var mergedIncidentId in mergeOwners.Where(id => id != incidentId))
            {
                ...
                merge.CommandText = """
                    ...
                    SET status='merged',
                        ...
                        updated_at_utc=$now
                    WHERE id=$merged_incident_id
                      AND incident_key LIKE 'batch-canary:%'
                      AND status='active';
                    """;
                merge.Parameters.AddWithValue("$incident_id", incidentId);
                merge.Parameters.AddWithValue("$merged_incident_id", mergedIncidentId);
                merge.Parameters.AddWithValue("$now", now);
                if (await merge.ExecuteNonQueryAsync(ct) != 1)
                    throw new InvalidOperationException($"verified relationship could not atomically merge replacement incident {mergedIncidentId}");
            }
            ...
            reason = mergeOwners.Count > 0
                ? $"independently verified relationship combined {mergeOwners.Count} replacement incident(s)"
                : created
        ...
        var metadata = JsonSerializer.Serialize(new
        {
            ...
            titleChanged = !string.IsNullOrWhiteSpace(existingTitle) && !string.Equals(existingTitle, intent.Title, StringComparison.Ordinal),
            mergedIncidentIds = mergeOwners,
