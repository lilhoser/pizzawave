...
public sealed class SetupJobService
{
    ...
    public async Task<SetupSdrDetectionDto> DetectSdrsAsync(CancellationToken ct)
    {
        ...
        foreach (var line in rtl.Stdout.Split('\n').Select(l => l.Trim()).Where(l => l.Length > 0))
        {
            ...
            var serial = match.Groups["serial"].Success ? match.Groups["serial"].Value.Trim() : string.Empty;
            if (string.IsNullOrWhiteSpace(serial))
                label = $"RTL-SDR #{index}";
            devices.Add(new SetupSdrDeviceDto(index, serial, string.IsNullOrWhiteSpace(label) ? $"RTL-SDR #{index}" : label, string.Empty, string.IsNullOrWhiteSpace(serial) ? "Serial was not reported; set unique serials before assigning SDRs." : string.Empty));
        }
        ...
        if (devices.Count == 0)
        {
            ...
            {
                devices.Add(new SetupSdrDeviceDto(index++, string.Empty, "Possible RTL-SDR USB device", line, "Serial unavailable from lsusb; run SDR priming and set/list serials before assigning."));
            }
        }

        var message = devices.Count == 0
            ? "No RTL-SDR devices were detected. Install rtl-sdr/usbutils/gqrx and verify USB passthrough/hardware."
            : $"Detected {devices.Count} possible RTL-SDR device(s)." + (stoppedTr ? " TR was stopped for detection and restarted afterward." : "");
    ...
    private async Task RunStopTrForCalibrationAsync(long jobId, CancellationToken ct)
    {
        ...
        await LogAsync(jobId, "info", "trunk-recorder is stopped. GQRX can now claim SDR devices. The sweep job will restart TR for each test pass.", ct);
        await _database.UpdateJobAsync(jobId, "completed", 1, 1, 0, "Stopped trunk-recorder for calibration.", false, true, ct);
        await _events.PublishAsync("job_updated", new { jobId, status = "completed" }, ct);
    }
    ...
    private async Task RunCalibrationSweepAsync(long jobId, JsonElement? parameters, CancellationToken ct)
    {
        ...
        var baseError = OptionalInt(root, "baseErrorHz", 0);
        var range = Math.Max(0, OptionalInt(root, "rangeHz", 1200));
        var step = Math.Max(1, OptionalInt(root, "stepHz", 300));
        var warmup = Math.Max(0, OptionalInt(root, "warmupSec", 20));
        var duration = Math.Max(1, OptionalInt(root, "durationSec", 240));
        var modulation = OptionalString(root, "modulation");
        if (string.IsNullOrWhiteSpace(modulation))
            modulation = "qpsk";
        var gain = OptionalString(root, "gain");
        var passCount = (int)Math.Floor((range * 2.0) / step) + 1;
        var estimatedSeconds = passCount * (warmup + duration);

        await LogAsync(jobId, "info", $"Running calibration sweep for {system} using source {sourceIndex} / rtl={serial}.", ct);
        await LogAsync(jobId, "info", $"This sweep has {passCount} pass(es) and is expected to take about {FormatElapsed(estimatedSeconds)}.", ct);
        await _database.UpdateJobAsync(jobId, "running", passCount, 0, 0, "Calibration sweep running.", false, false, ct);
        ...
        var args = new List<string>
        {
            ...
            "--base-error", baseError.ToString(),
            "--range-hz", range.ToString(),
            "--step-hz", step.ToString(),
            "--modulation", modulation,
            "--warmup-sec", warmup.ToString(),
        ...
        await _database.UpdateJobAsync(jobId, "completed", passCount, passCount, 0, "Calibration sweep completed. Review tr_tune output and apply findings only after confirming the best candidate.", false, true, ct);
        await _events.PublishAsync("job_updated", new { jobId, status = "completed" }, ct);
    }
    ...
    {
        var completed = 0;
        await RunCommandAsync(jobId, "bash", "-lc \"command -v rtl_test || true; command -v lsusb || true; command -v gqrx || true\"", ct);
        await _database.UpdateJobAsync(jobId, "running", 5, ++completed, 0, "Checked RTL-SDR and GQRX tool availability.", false, false, ct);
        if (!CommandExists("rtl_test") || !CommandExists("lsusb") || !CommandExists("gqrx"))
        {
            await LogAsync(jobId, "info", "Installing rtl-sdr, usbutils, and the available GQRX package with the setup helper.", ct);
        ...
        await _database.UpdateJobAsync(jobId, "running", 5, ++completed, 0, "RTL-SDR smoke check complete.", false, false, ct);
