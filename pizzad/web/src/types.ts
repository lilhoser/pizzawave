...
export type IncidentLinkShadowRunSummary = { runId: string; attempts: number; firstAttemptUtc?: string | null; lastAttemptUtc?: string | null; isConfiguredRun: boolean };
export type IncidentLinkShadowTotals = { attempts: number; candidateBackedAttempts: number; proposedLinks: number; admittedLinks: number; abstentions: number; invalidProposals: number; proposerErrors: number; averageProposerMilliseconds: number; maximumProposerMilliseconds: number; projectedEvents: number; projectedObservations: number; modelRequests: number; promptTokens: number; completionTokens: number; totalTokens: number };
export type IncidentLinkShadowAttempt = { sequence: number; recordedAtUtc: string; callId: number; candidateCount: number; decision: string; outcome: string; modelIdentity: string; projectionEventId: string; relationshipStatement: string; uncertainty: number; validationErrors: string[]; unresolvedQuestions: string[]; proposerMilliseconds: number; proposerError: string };
export type IncidentLinkShadowProjectedEvent = { projectionEventId: string; observationCount: number };
export type IncidentLinkShadowReport = { enabled: boolean; configuredRunId: string; selectedRunId: string; runs: IncidentLinkShadowRunSummary[]; totals: IncidentLinkShadowTotals; attempts: IncidentLinkShadowAttempt[]; projectedEvents: IncidentLinkShadowProjectedEvent[] };
