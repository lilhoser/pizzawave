...
export type IncidentBatchVerificationTotals = { enqueued: number; pending: number; verified: number; review: number; rejected: number; invalid: number; canaryPersisted: number; canaryConflicts: number; averageVerifierMilliseconds: number; maximumVerifierMilliseconds: number };
export type IncidentBatchVerificationItem = { requestSequence: number; requestId: string; enqueuedAtUtc: string; sourceLedgerEntryId: string; sourceProposalToken: string; candidateToken: string; proposedDisposition: string; outcome: string; completedAtUtc?: string | null; verifierMilliseconds: number; validationErrors: string[]; canaryOutcome: string; canaryIncidentId: number; canaryReason: string };
export type IncidentBatchVerificationReport = { enabled: boolean; runId: string; totals: IncidentBatchVerificationTotals; items: IncidentBatchVerificationItem[] };
