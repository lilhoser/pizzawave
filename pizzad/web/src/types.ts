...
export type IncidentBatchShadowTotals = { batches: number; newObservations: number; proposedEvents: number; newEvents: number; confirmedMemberships: number; provisionalAssociations: number; unresolvedObservations: number; invalidProposals: number; proposerErrors: number; averageProposerMilliseconds: number; maximumProposerMilliseconds: number; projectedEvents: number; operatorVisibleEvents: number };
export type IncidentBatchShadowAttempt = { sequence: number; recordedAtUtc: string; firstCallId: number; lastCallId: number; newObservationCount: number; candidateCount: number; proposedEventCount: number; newEventCount: number; confirmedMembershipCount: number; provisionalAssociationCount: number; unresolvedObservationCount: number; modelIdentity: string; promptIdentity: string; configurationIdentity: string; eventTitles: string[]; validationErrors: string[]; proposerMilliseconds: number; proposerError: string };
export type IncidentBatchShadowProjectedEvent = { projectionEventId: string; observationCount: number; title: string; summary: string; operatorVisible: boolean };
