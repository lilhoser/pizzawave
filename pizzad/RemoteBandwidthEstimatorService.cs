...
public sealed class RemoteBandwidthEstimatorService
{
    ...
    public async Task<RemoteBandwidthReportDto> BuildReportAsync(long start, long end, CancellationToken ct, int page = 1, int pageSize = 20)
    {
        ...
        var byTimeActivity = await _database.ListRemoteBandwidthTimeActivityBucketsAsync(startUtc, endUtc, bucketSeconds, ct);
        var entryTotal = await _database.CountRemoteBandwidthEntriesAsync(startUtc, endUtc, ct);
        var entries = await _database.ListRemoteBandwidthEntriesAsync(startUtc, endUtc, page, pageSize, ct);
