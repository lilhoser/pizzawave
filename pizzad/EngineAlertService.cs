...
public sealed class EngineAlertService
{
    ...
    public EngineAlertService(EngineConfig config, CredentialStore credentials, PoliceCodeService policeCodes, ILogger<EngineAlertService> logger)
    {
        _config = config;
    ...
    private void SendEmail(EngineAlertRule rule, EngineCall call, string transcription)
    {
        ...
        foreach (var recipient in Split(rule.Email))
        {
            ...
            SmtpEmailSender.SendHtml(
                ...
                _credentials.ResolveAlertEmailPassword());
        }
    }
