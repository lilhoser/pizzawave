...
public sealed partial class EngineDatabase
{
    ...
    public async Task InitializeAsync(CancellationToken ct)
    {
        ...
        await ExecuteNonQueryAsync(connection, IncidentExperimentSchemaSql, ct);
    ...
    public async Task<List<IncidentDto>> ListIncidentsAsync(long start, long end, CancellationToken ct)
    {
        ...
        await using var command = connection.CreateCommand();
        command.CommandText = """
            SELECT id, COALESCE(incident_key, ''), title, detail, COALESCE(category, 'other'), COALESCE(status, 'active'),
                   first_seen, last_seen, incident_score, COALESCE(previous_title, ''), COALESCE(title_updated_at_utc, ''),
                   COALESCE(merged_into_incident_id, 0)
            FROM incidents
            WHERE last_seen >= $start AND first_seen <= $end
        ...
        while (await reader.ReadAsync(ct))
        {
            ...
            incidents.Add(new IncidentDto
            {
                ...
                PreviousTitle = reader.GetString(9),
                TitleUpdatedAtUtc = reader.GetString(10),
                MergedIntoIncidentId = reader.GetInt64(11),
                Calls = []
            });
        }
    ...
    private async Task EnsureSchemaMigrationsAsync(SqliteConnection connection, CancellationToken ct)
    {
        ...
        await AddColumnIfMissingAsync(connection, "incidents", "previous_title", "TEXT NOT NULL DEFAULT ''", ct);
        await AddColumnIfMissingAsync(connection, "incidents", "title_updated_at_utc", "TEXT NOT NULL DEFAULT ''", ct);
        await AddColumnIfMissingAsync(connection, "incidents", "merged_into_incident_id", "INTEGER NOT NULL DEFAULT 0", ct);
