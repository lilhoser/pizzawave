...
public static class IncidentBatchRelationshipContract
{
    ...
    public const int MaximumCandidateCount =
        IncidentBatchContract.MaximumCandidateCount + IncidentBatchContract.MaximumNewObservationCount;
    public const int MaximumReturnedRelationships = MaximumSourceCount - 1;
    ...
    public const string ConfigurationToken = $"relationship-stage=source-isolated-v10;{AllObservationSourcesConfigurationToken};same-batch-peers=accepted-ordered-v2;pair-identity=compact-application-index-v1;evidence-identity=global-application-indices-v1;admission=concrete-cross-reference-v3;confirmation=conflict-free-v1;acceptance=per-relationship-v1;output=bounded-complete-v1";
...
{
    public const string PromptIdentity = "incident-batch-relationship-v12-compact-pair-indices";
        ...
        IReadOnlyList<IncidentBatchCandidate> candidates)
    {
        ...
        var sourceRows = sources.Select((item, index) => new
        {
            row = index + 1,
            token = item.SourceProposalToken,
            observations = item.NewObservationIds
        }).ToList();
        var candidateRows = candidates.Select((item, index) => new
        {
            row = index + 1,
            token = item.CandidateToken,
            observations = item.ObservationIds
        }).ToList();
        var sourceRowByToken = sourceRows.ToDictionary(item => item.token, item => item.row, StringComparer.Ordinal);
        var candidateRowByToken = candidateRows.ToDictionary(item => item.token, item => item.row, StringComparer.Ordinal);
        var source = new
        {
            evidence = evidenceCatalog.Select((item, index) => new object[]
            {
                index,
                item.ObservationId,
                item.TranscriptId,
            ...
            relationship_sources = sourceRows.Select(item => new object[]
            {
                item.row,
                item.token,
                item.observations
            }).ToList(),
            candidate_events = candidateRows.Select(item => new object[]
            {
                item.row,
                item.token,
                item.observations
            }).ToList(),
            eligible_relationship_pairs = eligiblePairs.Select((pair, index) => new
                object[]
                {
                    index,
                    sourceRowByToken[pair.SourceProposalToken],
                    candidateRowByToken[pair.CandidateToken]
                }).ToList()
        };
        var user = new StringBuilder();
        user.AppendLine("/no_think");
        user.AppendLine("Return only JSON matching the supplied schema.");
        user.AppendLine("Inspect every eligible pair for evidence-established relationships between application-owned relationship sources and supplied candidates using only their transcripts. Return every pair that meets the stated evidence threshold, up to the schema limit. Do not omit a clearly supported relationship merely to keep the response sparse. Return an empty relationships array only when no eligible pair meets the threshold.");
        ...
        user.AppendLine("Evidence is listed once as [evidence_index, observation_id, transcript_id, exact_quote]. Source and candidate rows refer to observation IDs, and eligible pairs are [relationship_pair_index, source_row, candidate_row]. Evaluate every listed pair and copy its integer pair index exactly.");
        ...
        user.AppendLine("Each returned relationship must select listed evidence indices belonging to the selected source and candidate boundaries. The application rejects cross-boundary evidence. Never generate, copy, edit, or paraphrase quote text.");
        user.AppendLine("Copy relationship_pair_index and evidence indices exactly.");
        user.AppendLine();
        ...
        return new IncidentBatchRelationshipPromptPayload(
            "You evaluate typed relationships between immutable source-grounded groups and unresolved singleton observations. You may attach evidence-cited relationships but cannot construct or rewrite events. Application code resolves global evidence indices, validates citation boundaries, and owns all state transitions.",
            user.ToString(),
            ResponseFormat(eligiblePairs, evidenceCatalog.Count));
    }

    private static object ResponseFormat(IReadOnlyList<IncidentBatchRelationshipPair> eligiblePairs, int evidenceCount)
    {
        object EvidenceIds() => new
        {
            ...
            uniqueItems = true,
            items = new { type = "integer", minimum = 0, maximum = evidenceCount - 1 }
        };
        ...
        return new
        {
            ...
            json_schema = new
            {
                name = "pizzawave_incident_batch_relationship_v12_compact_pair_graph",
                strict = true,
                schema = new
                {
                    type = "object",
                    ...
                    properties = new
                    {
                        relationships = new
                        {
                            ...
                            items = new
                            {
                                ...
                                properties = new
                                {
                                    relationship_pair_index = new
                                    {
                                        type = "integer",
                                        minimum = 0,
                                        maximum = eligiblePairs.Count - 1
                                    },
                                    ...
                                    source_evidence_indices = EvidenceIds(),
                                    candidate_evidence_indices = EvidenceIds(),
                                ...
                                required = new[] { "relationship_pair_index", "disposition", "relationship_statement", "uncertainty", "source_evidence_indices", "candidate_evidence_indices", "alternative_interpretations", "unresolved_questions" }
                            }
                        }
                    },
...
public sealed class OpenAiIncidentBatchRelationshipProposer : IIncidentBatchRelationshipProposer
{
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            {
                if (item.RelationshipPairIndex < 0 || item.RelationshipPairIndex >= eligiblePairs.Count)
                    throw new InvalidDataException(
                        $"Unknown batch relationship pair index '{item.RelationshipPairIndex}'.");
                var pair = eligiblePairs[item.RelationshipPairIndex];
                return new IncidentBatchRelationship(
                    ...
                    item.Uncertainty,
                    IncidentBatchConfirmationEvidenceCatalog.ResolveIndices(item.SourceEvidenceIndices, evidenceCatalog),
                    IncidentBatchConfirmationEvidenceCatalog.ResolveIndices(item.CandidateEvidenceIndices, evidenceCatalog),
    ...
    private sealed record RelationshipItemResponse(
        [property: JsonPropertyName("relationship_pair_index")] int RelationshipPairIndex,
        ...
        [property: JsonPropertyName("source_evidence_indices")] IReadOnlyList<int> SourceEvidenceIndices,
        [property: JsonPropertyName("candidate_evidence_indices")] IReadOnlyList<int> CandidateEvidenceIndices,
