...
The pull-request BVT lane builds the web UI, compiles the .NET solution, runs
fast unit tests, publish-checks linux-x64/linux-arm64, and compile-checks the
CUDA profile. Medium feature tests live behind the `Category=Feature` trait and
run from the scheduled/manual Feature Tests workflow.
...
See `docs/testing.md` for what belongs in each lane.
...
```powershell
.\scripts\deploy_pizzad_tar.ps1 -HostName ocroot@100.105.110.92 -SshKey $env:USERPROFILE\.ssh\pizzawave_rpi_ed25519 -Rid linux-arm64
