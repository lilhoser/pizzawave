...
public sealed class IncidentBatchConstructorPipelineTests
{
    ...
    [Fact]
    public void RollingSelectionReservesSlotsForLeastConsideredPendingEvidence()
    {
        var calls = Enumerable.Range(10, 6).Select(id => Call(id, $"Pending evidence {id}.", 1000 + id)).ToList();
        var newCall = Call(20, "New incident evidence.", 1020);
        var prior = PriorProjection(calls.Select(call => new IncidentBatchProjectionEvent(
            $"projection:event:{call.Id}", [$"call:{call.Id}"], string.Empty, string.Empty, false, false, ["ledger:prior"])).ToArray());
        var considered = new Dictionary<string, int>(StringComparer.Ordinal)
        {
            ["projection:event:10"] = 2,
            ["projection:event:11"] = 1,
            ["projection:event:12"] = 1
        };

        var selection = IncidentBatchLiveSelection.Build(
            [newCall],
            calls.Append(newCall).ToList(),
            [],
            prior,
            4,
            Now,
            includePendingCandidates: true,
            candidateConsiderationCounts: considered,
            maximumEvidencePerCandidate: 1);

        Assert.Equal(2, selection.Candidates.Count);
        Assert.Equal(["projection:event:13", "projection:event:14"], selection.Candidates.Select(item => item.ProjectionEventId));
        Assert.All(selection.Candidates, candidate => Assert.Single(candidate.ObservationIds));
        Assert.Equal(["call:20", "call:13", "call:14"], selection.NewObservationIds);
        Assert.All(selection.Candidates, candidate => Assert.False(candidate.OperatorVisible));
        Assert.Equal(3, selection.Bundle.Observations.Count);
    }
    ...
    [Fact]
    public async Task ExhaustiveSourceIntakeRejectsAnOmittedObservation()
    {
        var bundle = Bundle(
            Observation("call:1", "transcript:1", "Vehicle overturned with one person trapped."),
            Observation("call:2", "transcript:2", "Unit 12 is back in service."));
        var evidence = Citation("transcript:1", "Vehicle overturned with one person trapped.");
        var proposal = new IncidentBatchProposal(
            "proposal:source",
            Now,
            "application",
            ApplicationIncidentBatchExhaustiveSourceProposer.PromptIdentity,
            [Event("source:call:1", IncidentBatchEventDisposition.ProvisionalEvent, string.Empty, ["call:1"], "Vehicle overturned", "", [evidence], [])]);
        ...
        var result = await RunAsync(
            ...
            executionToken: $"{IncidentBatchContract.ObservationIsolatedOwnershipConfigurationToken};{IncidentBatchContract.ExhaustiveSourceIntakeConfigurationToken}");

        Assert.Contains(result.LedgerEntry.Entry.ProposalValidationErrors, error =>
            error.Contains("omitted new observation 'call:2'", StringComparison.Ordinal));
    }

    [Fact]
    public void BatchedStandaloneVerifierPreservesRowAndEvidenceOwnership()
    {
        var bundle = Bundle(
            Observation("call:1", "transcript:1", "Vehicle overturned with one person trapped."),
            Observation("call:2", "transcript:2", "Unit 12 is back in service."));
        var events = new[]
        {
            Event("source:1", IncidentBatchEventDisposition.NewEvent, string.Empty, ["call:1"], "Vehicle overturned", "", [Citation("transcript:1", "Vehicle overturned")], []),
            Event("source:2", IncidentBatchEventDisposition.ProvisionalEvent, string.Empty, ["call:2"], "Unit status", "", [Citation("transcript:2", "Unit 12 is back in service")], [])
        };
        const string json = """
            {"decisions":[
              [1,"v","Vehicle overturned with person trapped",[0],""],
              [2,"x","",[0],""]
            ]}
            """;

        var proposals = IncidentBatchStandaloneBatchVerification.Parse(bundle, events, json, "test-model", Now);

        Assert.Equal(2, proposals.Count);
        Assert.Equal(IncidentBatchConfirmationDecisionKind.Verify, proposals[0].Decision.Decision);
        Assert.Equal("transcript:1", Assert.Single(proposals[0].Decision.Evidence).TranscriptId);
        Assert.Equal(IncidentBatchConfirmationDecisionKind.Reject, proposals[1].Decision.Decision);
        Assert.Equal(IncidentBatchStandaloneBatchVerification.PromptIdentity, proposals[1].PromptIdentity);
    }

    [Fact]
    public void BatchedStandaloneVerifierRejectsShiftedRows()
    {
        var bundle = Bundle(Observation("call:1", "transcript:1", "Vehicle overturned."));
        var events = new[]
        {
            Event("source:1", IncidentBatchEventDisposition.NewEvent, string.Empty, ["call:1"], "Vehicle overturned", "", [Citation("transcript:1", "Vehicle overturned")], [])
        };
        ...
        Assert.Throws<InvalidDataException>(() =>
            IncidentBatchStandaloneBatchVerification.Parse(
                bundle,
                events,
                """{"decisions":[[2,"v","Vehicle overturned",[0],""]]}""",
                "test-model",
                Now));
    }

    [Fact]
    public void IncidentTitlePresentationTruncatesAtAWordBoundary()
    {
        const string title =
            "Reported vehicle collision with possible entrapment near the north entrance of the shopping center";

        var normalized = IncidentTitlePresentation.Normalize(title);

        Assert.True(normalized.Length <= IncidentBatchConfirmationContract.MaximumDisplayTitleLength);
        Assert.StartsWith(normalized, title, StringComparison.Ordinal);
        Assert.Equal(' ', title[normalized.Length]);
    }

    [Fact]
    public async Task DeterministicSourceIntakeCoversEveryObservationWithoutAModelDecision()
    {
        var bundle = Bundle(
            Observation("call:1", "transcript:1", "Vehicle overturned."),
            Observation("call:2", "transcript:2", "Unit 12 is back in service."));

        var proposal = await new ApplicationIncidentBatchExhaustiveSourceProposer(new FixedTimeProvider(Now))
            .ProposeAsync(bundle, ["call:1", "call:2"], [], CancellationToken.None);

        Assert.Equal("application", proposal.ModelIdentity);
        Assert.Equal(ApplicationIncidentBatchExhaustiveSourceProposer.PromptIdentity, proposal.PromptIdentity);
        Assert.Equal(2, proposal.Events.Count);
        Assert.All(proposal.Events, item => Assert.Equal(IncidentBatchEventDisposition.ProvisionalEvent, item.Disposition));
        Assert.Equal(["call:1", "call:2"], proposal.Events.SelectMany(item => item.NewObservationIds));
    }

    [Fact]
    public async Task DeterministicSourceIntakeRemainsAcceptedForBatchedStandaloneVerification()
    {
        var bundle = Bundle(
            Observation("call:1", "transcript:1", "Vehicle overturned."),
            Observation("call:2", "transcript:2", "Unit 12 is back in service."));
        var proposer = new ApplicationIncidentBatchExhaustiveSourceProposer(new FixedTimeProvider(Now));
        ...
        var result = await RunAsync(
            ...
            [],
            proposer,
            executionToken: $"{IncidentBatchContract.ObservationIsolatedOwnershipConfigurationToken};{IncidentBatchContract.ExhaustiveSourceIntakeConfigurationToken};{IncidentBatchExecutionArchitecture.StagedRelationshipAsynchronousConfirmationToken};{IncidentBatchStandaloneVerificationContract.ConfigurationToken};{IncidentBatchStandaloneBatchVerification.ConfigurationToken}");

        Assert.Empty(result.LedgerEntry.Entry.ProposalValidationErrors);
        Assert.Equal(2, IncidentBatchContract.AcceptedEvents(result.LedgerEntry.Entry).Count);
        var requests = IncidentBatchVerificationQueueContract.BuildRequests(result.LedgerEntry.Entry);
        Assert.Equal(2, requests.Count);
        Assert.All(requests, request => Assert.Equal(IncidentBatchVerificationKind.StandaloneEvent, request.Kind));
    }

    [Fact]
    public async Task RollingSinglePassPublishesModelPresentationWithoutASecondVerifier()
    {
        var bundle = Bundle(
            Observation("call:1", "transcript:1", "Vehicle overturned at 10 Oak Street."),
            Observation("call:2", "transcript:2", "Engine 4 responding to 10 Oak Street."));
        var item = Event(
            "rolling:event:1",
            IncidentBatchEventDisposition.NewEvent,
            string.Empty,
            ["call:1", "call:2"],
            "Vehicle rollover at 10 Oak Street",
            "A vehicle overturned and Engine 4 responded.",
            [Citation("transcript:1", "Vehicle overturned"), Citation("transcript:2", "Engine 4 responding")],
            []);
        ...
        var result = await RunAsync(
            ...
            [],
            new FixedProposer(Proposal([item])),
            executionToken: IncidentRollingHypothesis.ExecutionToken);

        var projected = Assert.Single(result.Projection.Projection.Events);
        Assert.Equal("Vehicle rollover at 10 Oak Street", projected.Title);
        Assert.Equal("A vehicle overturned and Engine 4 responded.", projected.Summary);
        Assert.True(projected.OperatorVisible);
        Assert.False(projected.OperatorReview);
    }

    [Fact]
    public async Task RollingSinglePassCanAttachNewEvidenceToCarriedForwardPendingEvidence()
    {
        var priorObservation = Observation("call:10", "transcript:10", "Medical response at 10 Oak Street.");
        var newObservation = Observation("call:11", "transcript:11", "Medic 4 arriving at 10 Oak Street.");
        var bundle = Bundle(priorObservation, newObservation);
        var prior = PriorProjection(new IncidentBatchProjectionEvent(
            "projection:event:pending", ["call:10"], string.Empty, string.Empty, false, false, ["ledger:prior"]));
        var candidate = new IncidentBatchCandidate("candidate:pending", "projection:event:pending", ["call:10"]);
        var item = Event(
            "rolling:event:1",
            IncidentBatchEventDisposition.ConfirmedMembership,
            candidate.CandidateToken,
            ...
            "Medic 4 responded to the medical call.",
            [Citation("transcript:11", "Medic 4 arriving")],
            [Citation("transcript:10", "Medical response")]);
        ...
        var result = await RunAsync(
            ...
            new FixedProposer(Proposal([item])),
            prior,
            IncidentRollingHypothesis.ExecutionToken);

        var projected = Assert.Single(result.Projection.Projection.Events);
        Assert.Equal(["call:10", "call:11"], projected.ObservationIds.Order(StringComparer.Ordinal));
        Assert.True(projected.OperatorVisible);
        Assert.False(projected.OperatorReview);
    }

    [Fact]
    public async Task RollingSinglePassCanRegroupPreviouslyPendingEvidence()
    {
        var first = Observation("call:10", "transcript:10", "Medical response at 10 Oak Street.");
        var second = Observation("call:11", "transcript:11", "Medic 4 arriving at 10 Oak Street.");
        var bundle = Bundle(first, second);
        var prior = PriorProjection(
            new IncidentBatchProjectionEvent("projection:event:pending:10", ["call:10"], string.Empty, string.Empty, false, false, ["ledger:prior"]),
            new IncidentBatchProjectionEvent("projection:event:pending:11", ["call:11"], string.Empty, string.Empty, false, false, ["ledger:prior"]));
        var item = Event(
            "rolling:event:1",
            IncidentBatchEventDisposition.NewEvent,
            string.Empty,
            ...
            "Medical response at 10 Oak Street",
            "Medic 4 responded to the medical call.",
            [Citation("transcript:10", "Medical response"), Citation("transcript:11", "Medic 4 arriving")],
            []);
        ...
        var result = await RunAsync(
            ...
            ["call:10", "call:11"],
            [
                new IncidentBatchCandidate("candidate:pending:10", "projection:event:pending:10", ["call:10"]),
                new IncidentBatchCandidate("candidate:pending:11", "projection:event:pending:11", ["call:11"])
            ],
            new FixedProposer(Proposal([item])),
            prior,
            IncidentRollingHypothesis.ExecutionToken);
    ...
    {
        var reconsidered = candidates
            .Where(candidate => !candidate.OperatorVisible)
            .SelectMany(candidate => candidate.ObservationIds.Select(observationId => (observationId, candidate.ProjectionEventId)))
            .ToDictionary(item => item.observationId, item => item.ProjectionEventId, StringComparer.Ordinal);
        var singletons = newObservationIds.Select(id => new IncidentBatchSingletonIdentity(
            id,
            reconsidered.GetValueOrDefault(id, $"projection:singleton:{id}"))).ToList();
