...
public sealed class IncidentBatchRelationshipTests
{
    ...
    public void PromptKeepsConstructionAndCandidateSourcesExplicitlySeparated()
    {
        ...
        Assert.Contains("Evidence is listed once", prompt.UserPrompt, StringComparison.Ordinal);
        ...
        Assert.Contains("evidence_index", prompt.UserPrompt, StringComparison.Ordinal);
        Assert.Equal(1, prompt.UserPrompt.Split("transcript:1", StringSplitOptions.None).Length - 1);
        Assert.Equal(1, prompt.UserPrompt.Split("transcript:2", StringSplitOptions.None).Length - 1);
        var schema = System.Text.Json.JsonSerializer.Serialize(prompt.ResponseFormat, EngineConfig.JsonOptions());
        Assert.Contains("relationship_pair_index", schema, StringComparison.Ordinal);
        Assert.Contains("source_evidence_indices", schema, StringComparison.Ordinal);
        Assert.Contains("candidate_evidence_indices", schema, StringComparison.Ordinal);
    ...
    [Fact]
    public void GlobalEvidenceIndicesRemainBoundedByTheSelectedPair()
    {
        var bundle = Bundle(
        ...
        var sourceCitation = evidenceCatalog.Single(item => item.ObservationId == "call:3");
        var candidateCitation = evidenceCatalog.Single(item => item.ObservationId == "call:1");
        var decision = new IncidentBatchRelationship(
            "event:follow-up",
            "candidate:first",
            IncidentBatchRelationshipDisposition.ConfirmedMembership,
            "The follow-up explicitly continues the first incident.",
            0,
            [new IncidentEventStateTranscriptCitation(sourceCitation.TranscriptId, sourceCitation.ExactQuote)],
            [new IncidentEventStateTranscriptCitation(candidateCitation.TranscriptId, candidateCitation.ExactQuote)],
            [],
            []);
        Assert.Equal("transcript:3", Assert.Single(decision.SourceEvidence).TranscriptId);
        Assert.Equal("transcript:1", Assert.Single(decision.CandidateEvidence).TranscriptId);
        var wrongBoundary = Proposal(decision with { SourceEvidence = decision.CandidateEvidence });
        var validation = IncidentBatchRelationshipContract.ValidateProposal(bundle, sources, candidates, wrongBoundary);
        Assert.False(validation.IsValid);
        Assert.Contains(validation.Errors, error => error.Contains("transcript outside", StringComparison.OrdinalIgnoreCase));
        Assert.Equal(
            "integer",
            item["properties"]!["source_evidence_indices"]!["items"]!["type"]!.GetValue<string>());
    ...
    public void RelationshipPromptTreatsAbstentionAsExpectedAndRequiresConcreteCrossReference()
    {
        ...
        Assert.Contains("Inspect every eligible pair", prompt.UserPrompt, StringComparison.Ordinal);
        Assert.Contains("Do not omit a clearly supported relationship", prompt.UserPrompt, StringComparison.Ordinal);
        Assert.Contains("only when no eligible pair meets the threshold", prompt.UserPrompt, StringComparison.Ordinal);
