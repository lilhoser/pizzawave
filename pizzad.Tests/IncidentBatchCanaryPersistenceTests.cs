...
public sealed class IncidentBatchCanaryPersistenceTests
{
    ...
    [Fact]
    public async Task VerifiedRelationshipCombinesOnlyReplacementOwnedIncidentsAndPreservesContinuity()
    {
        await using var fixture = await CanaryFixture.CreateAsync();
        var targetId = await fixture.Database.UpsertManagedIncidentAsync(new IncidentDto
        {
            IncidentKey = IncidentBatchCanaryContract.IncidentKey(fixture.RunId, "projection:event:existing"),
            Title = "Initial crash report",
            Detail = "Initial replacement-owned incident.",
            FirstSeen = fixture.Now.AddMinutes(-1).ToUnixTimeSeconds(),
            LastSeen = fixture.Now.AddMinutes(-1).ToUnixTimeSeconds(),
            Calls = [new IncidentCallDto(fixture.PriorCallId, fixture.Now.AddMinutes(-1).ToUnixTimeSeconds(), "", "")]
        }, CancellationToken.None);
        var sourceId = await fixture.Database.UpsertManagedIncidentAsync(new IncidentDto
        {
            IncidentKey = IncidentBatchCanaryContract.IncidentKey(fixture.RunId, "projection:event:source"),
            Title = "Critical injury report",
            Detail = "Later replacement-owned incident.",
            FirstSeen = fixture.Now.ToUnixTimeSeconds(),
            LastSeen = fixture.Now.ToUnixTimeSeconds(),
            Calls = [new IncidentCallDto(fixture.SourceCallId, fixture.Now.ToUnixTimeSeconds(), "", "")]
        }, CancellationToken.None);
        var staged = await fixture.StageVerifiedMembershipAsync();
        ...
        Assert.Equal(IncidentBatchCanaryCommitOutcome.Persisted, appended.Commit.Commit.Outcome);
        Assert.Equal(targetId, appended.Commit.Commit.IncidentId);
        var incidents = await fixture.Database.ListIncidentsAsync(
            fixture.Now.AddMinutes(-5).ToUnixTimeSeconds(),
            fixture.Now.AddMinutes(5).ToUnixTimeSeconds(),
            CancellationToken.None);
        var target = Assert.Single(incidents, incident => incident.Id == targetId);
        var source = Assert.Single(incidents, incident => incident.Id == sourceId);
        Assert.Equal("White-truck crash", target.Title);
        Assert.Equal("Initial crash report", target.PreviousTitle);
        Assert.False(string.IsNullOrWhiteSpace(target.TitleUpdatedAtUtc));
        Assert.Equal([fixture.PriorCallId, fixture.SourceCallId], target.Calls.Select(call => call.CallId).Order());
        Assert.Equal("merged", source.Status);
        Assert.Equal(targetId, source.MergedIntoIncidentId);
    }
